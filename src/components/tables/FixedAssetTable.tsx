...
interface FixedAssetTableProps {
  items: FixedAsset[];
  rooms: Room[];
  onEdit: (item: FixedAsset) => void;
  onDelete: (item: FixedAsset) => void;
}

const conditionLabels: Record<FixedAsset["condition"], string> = {
  baik: "Baik",
  cukup: "Cukup",
  rusak: "Rusak",
};

const FixedAssetTable = ({ items, rooms, onEdit, onDelete }: FixedAssetTableProps) => {
  ...
  const columns = [
    {
      ...
      label: "No",
      sortable: true,
      render: (value: any, item: FixedAsset, index?: number) => index !== undefined ? index + 1 : 1,
    },
    {
      key: "photoUrl",
      label: "Foto",
    ...
    },
    {
      ...
      label: "Nama",
      sortable: true,
      render: (value: string, item: FixedAsset) => (
    ...
    },
    {
      ...
      label: "Kondisi",
      sortable: true,
      render: (value: FixedAsset["condition"]) => (
  ...
  const renderActions = (item: FixedAsset) => (
      ...
      <Dialog>
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="pb-6">
            <DialogTitle className="text-xl font-bold">Detail Aset Tetap</DialogTitle>
            <p className="text-sm text-muted-foreground mt-1">Informasi lengkap aset tetap aset tetap</p>
          </DialogHeader>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-muted/20 rounded-lg p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Kode Aset Tetap</label>
                    <p className="font-medium font-mono text-sm">{item.code}</p>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Nama Aset Tetap</label>
              ...
              <div className="bg-muted/20 rounded-lg p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col space-y-2">
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Kondisi Aset Tetap</label>
                    <Badge
                      className="w-fit"
              ...
              <div className="bg-muted/20 rounded-lg p-4 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Status Aset Tetap</label>
                ...
                <div className="flex items-center justify-center">
                  {item.photoUrl ? (
                    ...
                    <div className="w-full h-48 rounded-lg bg-muted border-2 border-dashed border-muted-foreground/30 flex flex-col items-center justify-center text-muted-foreground">
                      ...
                      <span className="text-xs">Foto aset tetap tidak tersedia</span>
                    </div>
                  )}
                </div>
              </div>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base font-semibold">Daftar Aset Tetap</CardTitle>
        <p className="text-sm text-muted-foreground">Pantau kondisi dan lokasi aset tetap sekolah.</p>
      </CardHeader>
      <CardContent>
        <DataTable
          data={items}
          columns={columns}
          searchPlaceholder="Cari aset tetap..."
          actions={renderActions}
          exportable={true}
          emptyMessage="Belum ada data aset tetap."
        />
      </CardContent>
    </Card>
  );
