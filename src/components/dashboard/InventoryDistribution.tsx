...
const InventoryDistribution = ({ items, rooms }: InventoryDistributionProps) => {
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        <CardTitle className="text-base font-semibold">Sebaran Barang per Ruang</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {totalsByRoom.length === 0 ? (
          <p className="text-sm text-muted-foreground">Belum ada data ruangan.</p>
        ) : (
            ...
            return (
              <div key={entry.room.id} className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">{entry.room.name}</p>
                  <span className="text-xs text-muted-foreground">{entry.total} unit</span>
                </div>
                <div className="h-2 w-full rounded-full bg-muted">
                  <motion.div
                    ...
                    className={cn("h-2 rounded-full", barColors[index % barColors.length])}
                  />
                </div>
